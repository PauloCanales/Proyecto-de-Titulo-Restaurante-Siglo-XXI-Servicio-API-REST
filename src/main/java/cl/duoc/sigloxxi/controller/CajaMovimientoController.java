...
import cl.duoc.sigloxxi.repository.CajaMovimientoRepository;
...
@RestController
@RequestMapping("/api/caja-movimiento-controller")
public class CajaMovimientoController {
	...
	CajaRepository cajaRepository;
	  ...
	  */
	@GetMapping(path = "/list", name = "CajaMovimiento: " + NameMethods.MENSAJE_LISTAR)
	public ResponseEntity<?> getAll() {
		try {
			List<CajaMovimiento> listTemp = cajaMovimientoRepository.findAll();
			if (!listTemp.isEmpty()) {
	  ...
	  * @return devuelve un objeto json con la lista de registros
	  */
	@GetMapping(path = "/list-active", name = "CajaMovimiento: " + NameMethods.MENSAJE_LISTAR_ACTIVO)
	public ResponseEntity<?> getAllActive() {
		try {
			

			List<CajaMovimiento> listTemp = cajaMovimientoRepository.findAllActive();
			if (!listTemp.isEmpty()) {
				return new ResponseEntity<Object>(listTemp, HttpStatus.OK);
			} else {
				return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_REGISTRO_NO_ENCONTRADO, 204),
	  ...
	  * @return devuelve un objeto json con la lista de registros
	  */
	@GetMapping(path = "/list-inactive", name = "CajaMovimiento: " + NameMethods.MENSAJE_LISTAR_INACTIVO)

	public ResponseEntity<?> getAllInactive() {
		
		try {
		
			List<CajaMovimiento> listTemp = cajaMovimientoRepository.findAllInactive();
			if (!listTemp.isEmpty()) {
				return new ResponseEntity<Object>(listTemp, HttpStatus.OK);
			} else {
				return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_REGISTRO_NO_ENCONTRADO, 204),
	...
	@PostMapping(path = "/new", name = "CajaMovimiento: " + NameMethods.MENSAJE_NEW)
	public ResponseEntity<?> create(@RequestBody CajaMovimiento cajaMovimiento, @RequestHeader HttpHeaders headers) {
		
		//Validación Nombre
		if(cajaMovimiento.getNombre() == null) {
			return new ResponseEntity<Object>(new CustomResponse(Constants.VALID_NOMBRE_REQUERIDO, 400),
					HttpStatus.BAD_REQUEST);
		}	
		
		//Validación Valor
		if(cajaMovimiento.getValor() == null) {
			return new ResponseEntity<Object>(new CustomResponse(Constants.VALID_VALOR_REQUERIDO, 400),
					HttpStatus.BAD_REQUEST);
		}		
		
		//Validación FkTipoMovimiento
		if(cajaMovimiento.getFkTipoMovimiento() == null) {
			return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_REGISTRO_NO_ENCONTRADO, 400),
					HttpStatus.BAD_REQUEST);
		}	
		
		//Validación FkCaja
		if(cajaMovimiento.getFkCaja() == null) {
			return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_REGISTRO_NO_ENCONTRADO, 400),
					HttpStatus.BAD_REQUEST);
		}
		
		try {
			if (cajaMovimientoRepository.findByNombre(cajaMovimiento.getNombre()) == null) {
	
		
		Optional<TipoMovimiento> tipoMovimiento = tipoMovimientoRepository.findById(cajaMovimiento.getFkTipoMovimiento());
		...
		Optional<Caja> caja = cajaRepository.findById(cajaMovimiento.getFkCaja());
		//Validación Caja
		if(caja.get() == null) {
			return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_REGISTRO_NO_ENCONTRADO, 400),
					HttpStatus.BAD_REQUEST);
		...
		if(tipoMovimiento.get()!=null) {
			...
			cajaMovimiento.setTipoMovimiento(tipoMovimiento.get());
			cajaMovimiento.setCaja(caja.get());
		
            cajaMovimiento.getIdCajaMovimiento();
            cajaMovimiento.getNombre();
            cajaMovimiento.setEstado(1l);
            cajaMovimiento.getValor();
            cajaMovimiento.getCreatedAt();
			
			
			
			
			cajaMovimientoRepository.save(cajaMovimiento);
	...
	@GetMapping(path = "/get/{id_caja_movimiento}", name = "CajaMovimiento: " + NameMethods.MENSAJE_GET_BY_ID)
	public ResponseEntity<?> getById(@PathVariable(value = "cajaMovimiento") Long idCajaMovimiento) {
		try {
			CajaMovimiento cajaMovimiento = cajaMovimientoRepository.findOneActive(idCajaMovimiento);
			...
			return ResponseEntity.ok().body(cajaMovimiento);
		} catch (Exception e) {
			return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_ERROR_SERVER, 500),
					HttpStatus.INTERNAL_SERVER_ERROR);
	  ...
	  */	
	@PutMapping(path = "/update/{id_caja_movimiento}", name = "CajaMovimiento: " + NameMethods.MENSAJE_UPDATE)
	public ResponseEntity<?> update(@PathVariable(value = "id_caja_movimiento") Long idCajaMovimiento, @Valid @RequestBody CajaMovimiento cajaMovimientoDetails) {
		try {

			Optional<CajaMovimiento> cajaMovimiento = cajaMovimientoRepository.findById(idCajaMovimiento);
			
			if (cajaMovimiento.get() == null) {
				return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_REGISTRO_NO_ENCONTRADO, 404),
						HttpStatus.NOT_FOUND);
			}
			cajaMovimiento.get().setNombre(cajaMovimientoDetails.getNombre());
	
			cajaMovimientoRepository.save(cajaMovimiento.get());
	...
	@DeleteMapping(path = "/delete/{id_caja_movimiento}", name = "CajaMovimiento: " + NameMethods.MENSAJE_DELETE)
	public ResponseEntity<?> delete(@PathVariable(value = "id_caja_movimiento") Long idCajaMovimiento) {
		try {
			CajaMovimiento cajaMovimiento = cajaMovimientoRepository.findOneActive(idCajaMovimiento);
			if (cajaMovimiento == null) {
				return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_REGISTRO_NO_ENCONTRADO, 404),
