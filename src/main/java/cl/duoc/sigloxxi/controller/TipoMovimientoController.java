...
@RestController
@RequestMapping("/api/tipo-movimiento-controller")
public class TipoMovimientoController {
	...
	TipoMovimientoRepository tipoMovimientoRepository;
	  ...
	  */
	@GetMapping(path = "/list", name = "TipoMovimiento: " + NameMethods.MENSAJE_LISTAR)
	public ResponseEntity<?> getAll() {
		try {
			List<TipoMovimiento> listTemp = tipoMovimientoRepository.findAll();
			if (!listTemp.isEmpty()) {
	  ...
	  * @return devuelve un objeto json con la lista de registros
	  */
	@GetMapping(path = "/list-active", name = "TipoMovimiento: " + NameMethods.MENSAJE_LISTAR_ACTIVO)
	public ResponseEntity<?> getAllActive() {
		try {
			

			List<TipoMovimiento> listTemp = tipoMovimientoRepository.findAllActive();
			if (!listTemp.isEmpty()) {
				return new ResponseEntity<Object>(listTemp, HttpStatus.OK);
			} else {
				return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_REGISTRO_NO_ENCONTRADO, 204),
	  ...
	  * @return devuelve un objeto json con la lista de registros
	  */
	@GetMapping(path = "/list-inactive", name = "TipoMovimiento: " + NameMethods.MENSAJE_LISTAR_INACTIVO)

	public ResponseEntity<?> getAllInactive() {
		
		try {
		
			List<TipoMovimiento> listTemp = tipoMovimientoRepository.findAllInactive();
			if (!listTemp.isEmpty()) {
				return new ResponseEntity<Object>(listTemp, HttpStatus.OK);
			} else {
				return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_REGISTRO_NO_ENCONTRADO, 204),
	...
	@PostMapping(path = "/new", name = "TipoMovimiento: " + NameMethods.MENSAJE_NEW)
	public ResponseEntity<?> create(@Valid @RequestBody TipoMovimiento tipoMovimiento) {
		...
		try {
			if (tipoMovimientoRepository.findByNombre(tipoMovimiento.getNombre()) == null) {
	
				tipoMovimiento.setEstado(1L);
	...
	@GetMapping(path = "/get/{id}", name = "TipoMovimiento: " + NameMethods.MENSAJE_GET_BY_ID)
	public ResponseEntity<?> getById(@PathVariable(value = "id") Long idTipoMovimiento) {
		try {
			TipoMovimiento tipoMovimiento = tipoMovimientoRepository.findOneActive(idTipoMovimiento);
			if (tipoMovimiento == null) {
				return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_REGISTRO_NO_ENCONTRADO, 404),
						HttpStatus.NOT_FOUND);
			}
			return ResponseEntity.ok().body(tipoMovimiento);
		} catch (Exception e) {
			return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_ERROR_SERVER, 500),
					HttpStatus.INTERNAL_SERVER_ERROR);
	...
	@PutMapping(path = "/update/{id}", name = "TipoMovimiento: " + NameMethods.MENSAJE_UPDATE)
	public ResponseEntity<?> update(@PathVariable(value = "id") Long idTipoMovimiento, @Valid @RequestBody TipoMovimiento tipoMovimientoDetails) {
		try {

			Optional<TipoMovimiento> tipoMovimiento = tipoMovimientoRepository.findById(idTipoMovimiento);
			
			if (tipoMovimiento.get() == null) {
				return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_REGISTRO_NO_ENCONTRADO, 404),
						HttpStatus.NOT_FOUND);
			}
			tipoMovimiento.get().setNombre(tipoMovimientoDetails.getNombre());
	
			tipoMovimientoRepository.save(tipoMovimiento.get());
	...
	@DeleteMapping(path = "/delete/{id}", name = "TipoMovimiento: " + NameMethods.MENSAJE_DELETE)
	public ResponseEntity<?> delete(@PathVariable(value = "id") Long idTipoMovimiento) {
		try {
			TipoMovimiento tipoMovimiento = tipoMovimientoRepository.findOneActive(idTipoMovimiento);
			if (tipoMovimiento == null) {
				return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_REGISTRO_NO_ENCONTRADO, 404),
