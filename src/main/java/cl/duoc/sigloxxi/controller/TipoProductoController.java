...
@RestController
@RequestMapping("/api/tipoproducto-controller")
public class TipoProductoController {
	...
	TipoProductoRepository tipoproductoRepository;
	  ...
	  */
	@GetMapping(path = "/list", name = "TipoProducto: " + NameMethods.MENSAJE_LISTAR)
	public ResponseEntity<?> getAll() {
		try {
			List<TipoProducto> listTemp = tipoproductoRepository.findAll();
			if (!listTemp.isEmpty()) {
	  ...
	  * @return devuelve un objeto json con la lista de registros
	  */
	@GetMapping(path = "/list-active", name = "TipoProducto: " + NameMethods.MENSAJE_LISTAR_ACTIVO)
	public ResponseEntity<?> getAllActive() {
		try {
			

			List<TipoProducto> listTemp = tipoproductoRepository.findAllActive();
			if (!listTemp.isEmpty()) {
				return new ResponseEntity<Object>(listTemp, HttpStatus.OK);
			} else {
				return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_REGISTRO_NO_ENCONTRADO, 204),
	  ...
	  * @return devuelve un objeto json con la lista de registros
	  */
	@GetMapping(path = "/list-inactive", name = "TipoProducto: " + NameMethods.MENSAJE_LISTAR_INACTIVO)

	public ResponseEntity<?> getAllInactive() {
		
		try {
		
			List<TipoProducto> listTemp = tipoproductoRepository.findAllInactive();
			if (!listTemp.isEmpty()) {
				return new ResponseEntity<Object>(listTemp, HttpStatus.OK);
			} else {
				return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_REGISTRO_NO_ENCONTRADO, 204),
	...
	@PostMapping(path = "/new", name = "TipoProducto " + NameMethods.MENSAJE_NEW)
	public ResponseEntity<?> create(@RequestBody TipoProducto tipoProducto, @RequestHeader HttpHeaders headers) {
	...
	@GetMapping(path = "/get/{id}", name = "TipoProducto: " + NameMethods.MENSAJE_GET_BY_ID)
	public ResponseEntity<?> getById(@PathVariable(value = "id") Long id) {
		try {
			TipoProducto tipoProducto = tipoproductoRepository.findOneActive(id);
			if (tipoProducto == null) {
				return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_REGISTRO_NO_ENCONTRADO, 404),
						HttpStatus.NOT_FOUND);
			}
			return ResponseEntity.ok().body(tipoProducto);
		} catch (Exception e) {
			return new ResponseEntity<Object>(new CustomResponse(Constants.MENSAJE_ERROR_SERVER, 500),
					HttpStatus.INTERNAL_SERVER_ERROR);
	  ...
	  */	
	@PutMapping(path = "/update/{id}", name = "TipoProducto: " + NameMethods.MENSAJE_UPDATE)
	public ResponseEntity<?> update(@PathVariable(value = "id") Long id, @Valid @RequestBody TipoProducto tipoproductoDetails) {
		try {
			...
			tipoProducto.get().setNombre(tipoproductoDetails.getNombre());
	
			tipoproductoRepository.save(tipoProducto.get());
	...
	@DeleteMapping(path = "/delete/{id}", name = "TipoProducto: " + NameMethods.MENSAJE_DELETE)
